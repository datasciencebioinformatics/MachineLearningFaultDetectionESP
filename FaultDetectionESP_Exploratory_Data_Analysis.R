...
# Rename collumns of spectrum data
spectrum_features_data<-spectrum_features_data[,-which(colnames(spectrum_features_data) %in% c("id","esp_id","label","esp_with_label_id"))]
...
# Plot pca's
PCA_of_spectral_data_label            <-autoplot(model.pca, data = df_esp_frequency, colour = 'label') + theme_bw()  + theme(legend.position="bottom")
PCA_of_spectral_data_esp_id           <-autoplot(model.pca, data = df_esp_frequency, colour = 'esp_id') + theme_bw() + theme(legend.position="bottom")
